...
单位随模型保存，文本层用**双单位代码**记录 `值 <维度> <子码>`：
**第一个是维度编号（温/压/流量…），第二个才是具体单位。**

| 代码 | 含义 |
|-|-|
| `<22> <1>` | 温度，**K**（SI 集） |
| `<22> <2>` | 温度，**°F**（ENG 集，TEMP/T1 等） |
| `<22> <3>` | 温度，**K**（MET 集——注意 MET 是 K 不是 °C） |
| `<22> <4>` | 温度，**°C**（METCBAR / 自定义 °C 集） |
| `<20> <1>` | 压力，**Pa** |
...
| `<-89> <3>` | 摩尔流量基准（TOTAL / MOLE-FLOW）；ENG 体系下为 lbmol/hr |
| `<75> <5>` | 压降，psi（DP-COL） |

**教训**：`<20> <5>`(bar) 与 `<20> <2>`(psi) 差 **14.5038 倍**，压力写错一个代码塔压差一个量级。

⚠️ **两条容易犯的错**：

1. **不能由单位集推断代码**——实测存在 INSET=SI 的文件里某个 TEMP 却是 `<22><2>`（°F）。
   单个参数可以覆盖全局单位集。**必须读目标数值自己身上那两个尖括号。**
2. **`<22>` 不等于 °F**——它只是"温度"维度，单位看子码。
   把 METCBAR 模型的 `<22><4>` 当 °F 处理，215 °C 会变成 101.7 °C。

完整字典与统计依据见 **`references/units-codebook.md`**（203 个官方 bkp 实测）。
改文本前用 `scripts/aspen_units.py probe <bkp>` 实跑一次；
用 `sub_value()` 替换，代码不符时会抛错而不是静默写错。
